...
export default function ChartUsersTwo({ data }: any) {
        ...
        if (active && payload && payload.length) {
            ...
            return (
                <div className="dark:bg-zinc-900 bg-white p-4 rounded-md flex flex-col gap-y-1 border border-slate-300 dark:border-zinc-700">
                    <p className='text-gray-900 dark:text-gray-50 text-sm font-medium'>Статистика за {formatDate(new Date(dataPoint.payload.date))}</p>
    ...
    return (
        <div className="w-full h-fit" ref={myDivRef}>
            <LineChart width={divWidth - 32} height={240} data={data}>
                ...
                <Legend />
                <Line type="monotone" dataKey="loyal" stroke="#5AD6CC" name="С лояльностью" />
                <Line type="monotone" dataKey="no_loyal" stroke="#805AD5" name="Без лояльности" />
